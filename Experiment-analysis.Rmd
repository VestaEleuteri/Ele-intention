...
```{r load-libraries}
rm(list=ls())
load("Experiment analysis.Rdata") #Add Rdata

#Packages for directory and data sorting
library(here) #to set plot directory
library(reshape2) 
library(dplyr)
library(tidyr) #for pivoting wide to/from long

#Packages for analyses
library(lme4) #glmms
library(emmeans) #posthoc comparisons
library(Matrix) #for glmms
...
```{r set-environment}
here::i_am(file.path("Experiment analysis.Rmd")) #set working directory
```
...
descr_gest_del_cond <- xdata %>%
  ...
  dplyr::summarise(
    mean_value_gtok = round(mean(Gesture_Record_Count, na.rm = TRUE), 3),
    sd_value_gtok = round(sd(Gesture_Record_Count, na.rm = TRUE), 3), 
    min_value_gtok = round(min(Gesture_Record_Count, na.rm = TRUE), 3),
    max_value_gtok = round(max(Gesture_Record_Count, na.rm = TRUE), 3),
    mean_value_gype = round(mean(Gesture_Type_Count, na.rm = TRUE), 3),
    sd_value_gtype = round(sd(Gesture_Type_Count, na.rm = TRUE), 3),
    min_value_gtype = round(min(Gesture_Type_Count, na.rm = TRUE), 3),
    max_value_gtype = round(max(Gesture_Type_Count, na.rm = TRUE), 3)
  )
...
variance_value_sr <- var(xdata$Gesture_Record_Count, na.rm = TRUE) #very different, suggests overdispersion so we used a negative binomial model
plot(table(xdata$Gesture_Record_Count)) 
```
Issue of overdispersion so we used a negative binomial model

##Negative binomial GLMM Number of gesture tokens 
...
table(full.gtok.stab$detailed$lme4.warnings) 
table(full.gtok.stab$detailed$opt.warnings) 
...
N.B. Single variables in interaction results (and Age, Sex, controls) should not be interpreted (Delivery_Phase and Experimental_Condition).
...
plot.res.int.gtok
ggsave(here::here("plots", "Mod_ngest_tokens_int.png"), width = 10, height = 6)
```
...
## Plot Percentage of novel or repeated gesture types during post-delivery across conditions 
...
xdata2 <- subset(xdata2, ! Novel_Gest_Types_Used %in% c("NV")) #drop NV
...
# View the first few rows of the reshaped dataframe
head(xdata3)
```
...
variance_value_sr <- var(xdata3$Gesture_Record_Count, na.rm = TRUE) #very different, suggests overdispersion so use negative binomial model again
...
##Negative binomial GLMM with 0 inflation - Number of gesture tokens 
...
cis.nrtype.gtok=confint(full.nrtype.gtok, method = "profile")
...
variance_value_st <- var(xdata$Gesture_Type_Count, na.rm = TRUE) #similar so does not seem overdispersed, doublechecked below
...
table(xdata$Gesture_Record_Count)
table(xdata$Gesture_Type_Count, xdata$Gesture_Record_Count) 
...
save.image("Experiment analysis.Rdata") #Add Rdata
```
